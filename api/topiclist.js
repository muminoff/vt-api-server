var getTopicListQuery = "SELECT id, title, body, parent_room, closed, json_build_object('id', owner, 'username', (SELECT username FROM users WHERE id=owner)) as owner, attrs, (EXTRACT(epoch FROM created_at) * 1000)::int8 as created_at, (EXTRACT(epoch FROM closed_at) * 1000)::int8 as closed_at FROM topics WHERE parent_room=$1 AND closed=false AND id in (SELECT topic_id FROM subscribers WHERE user_id=$2)";
